                ...
                <div class="card">
                    <div class="card-body">
                        <div class="dropdown float-end">
                            <button type="button" class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target=".brandModal">Tambah</button>
                        </div>
                        <h4 class="card-title mb-4">Merek</h4>
                                    ...
                                    @if (count($brand) > 0)
                                        @foreach ($brand as $row)
                                            <tr>
                                                ...
                                                <td>{{ $row->id }}</td>
                                                <td>
                                                    <img src="{{ asset('storage/brands/'. $row->image) }}" alt="brand-img" class="avatar-lg">
                                                </td>
                                                <td>{{ $row->title }}</td>
                                                ...
                                                <td>
                                                    <div>
                                                        <form method="POST" action="{{ route('brand.destroy', $row->id) }}">
                                                            @csrf @method('DELETE')
                                                            <a href="{{ route('brand.edit', $row->id) }}" class="text-primary"><i class="mdi mdi-pencil font-size-18"></i></a>
                        ...
                        <div class="d-flex justify-content-between align-items-center px-3">
                            <span class="text-bold">Menampilkan {{ $brand->firstItem() }} hingga {{ $brand->lastItem() }} dari {{ $brand->total() }} entri</span>
...
@section('modal')
<div class="modal fade brandModal" tabindex="-1" role="dialog" aria-labelledby="brandModalLabel" aria-hidden="true">
