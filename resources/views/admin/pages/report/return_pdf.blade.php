...
<body>
    <h5>Laporan Pengembalian Periode ({{ $date[0] }} - {{ $date[1] }})</h5>
    <hr>
        ...
        <tbody>
            ...
            @forelse ($orders as $row)
                <tr>
                    ...
                    <td>Rp {{ number_format($row->subtotal + $row->cost) }}</td>
                    <td>{{ $row->created_at->format('d-m-Y') }}</td>
                </tr>

                @php $total += $row->subtotal + $row->cost @endphp
            @empty
            <tr>
